...
def language_recovery(results: Path, output: Path) -> None:
    ...
    width = 0.30
    fig, axes = plt.subplots(1, 2, figsize=(7.1, 2.75), gridspec_kw={"width_ratios": [1.25, 1]})
    ax = axes[0]
    ax.bar(x - width / 2, parser["parser_file_r5"], width, label="Diff parser", color="#1b4965")
    ...
    for i, ceiling in enumerate(parser["file_ceiling_r5"]):
        ax.plot(
            [i - 0.38, i + 0.38], [ceiling, ceiling], color="#555555", linestyle="--", linewidth=1
        )
    ...
    for i, language in enumerate(languages):
        ...
        ax.scatter(raw[i], i, color="#2a9d8f", marker="o", label="Raw R@5" if i == 0 else None)
        ax.scatter(
            normalized[i],
            i,
            color="#e9c46a",
...
def model_deltas(results: Path, output: Path) -> None:
    ...
    ax.scatter(
        deltas["production"], y + 0.13, color="#1b4965", marker="o", label="Production paths"
    )
    ax.scatter(deltas["test"], y - 0.13, color="#e76f51", marker="s", label="Test paths")
